...
const DEFAULT_PROD_API = 'https://shiksha-ai-1.onrender.com';

const API_URL = window.SHIKSHA_API_URL || localStorage.getItem('shiksha_api') || (isApiOrigin ? location.origin : (isLocalhost ? 'https://shiksha-ai-1.onrender.com' : DEFAULT_PROD_API));
const token = () => localStorage.getItem('shiksha_token');
async function request(path, options = {}) { const headers = options.headers || {}; if (token()) headers.Authorization = `Bearer ${token()}`; let res; try { res = await fetch(API_URL + path, { ...options, headers }) } catch (err) {
